...
import { Users, Projects, Clients, Branches, ProjectUser, ProjectClient, Events } from '@/schemas';
import ms from 'ms';
...
void (async function () {
  ...
  for (const {id, file} of files) {
    ...
    let content;
    try {
      content = (await fs.readFile(`${directory}/${file}`, 'utf-8')).replace(/"{2}/g, "\\\"");
      records = parse(content, { columns: true, skip_empty_lines: true, trim: true, delimiter: ',', escape: '\\', quote: '"'});
    } catch (error: any) {
      ...
      case 'events': {
        for (const record of records) {
          await db.insert(Events).values({
            ...
            set: skipIfEmpty({
                ...record, 
                startAt: record.startAt ? new Date(record.startAt): undefined,
                endAt: record.endAt ? new Date(record.endAt) : undefined,
                rrule: record.rrule ? JSON.parse(record.rrule) : undefined,
            }),
          });
