	...
	const timeMarkers = useMemo(
		() =>
			timeList.map(({ time, title }) => (
				<div
					key={time}
					className='text-xs lg:text-sm rtl:text-right text-gray-5 text-center text-ellipsis'
			...
			<div className={clsx('flex relative w-fit min-w-full')}>
				...
				{dates.map((date, index) => (
					<div
						...
						key={date}
						className={clsx(
							'relative -mt-2 w-full text-sm font-normal rtl:text-right text-gray-5 text-center text-ellipsis',
							'cursor-copy',
